...
_mode = _this param [0,"config",[""]];
_sides = _this param [1,[0,1,2,3],[[]]];
_patches = _this param [2,[],[[]]];
_allVehicles = _this param [3,true,[true]];
	...
	case "config": {
		...
		_cfgVehiclesCount = count _cfgVehicles;
		{
			...
			if (
				...
				_text = _text + "| " + _textWeapons + _br + _br;
				_text = _text + "| " + _textMagazines + _br + _br;
			...
			if (
				...
				if (!(_model in _restrictedModels) && inheritsfrom _x != _cfgAll && {_class iskindof _x} count _blacklist == 0) then {
					...
					if (_capture) then {
						...
						//screenshot format ["CfgVehicles_%1.png", _class]; //vanilla screenshot command images seem too washed out
						
						private _fileName = format ["%1.png", _class];
						private _outputPath = if_savePath + _fileName;
						diag_log format ["Image saved to: %1", _outputPath];
						"scr_cap" callExtension _outputPath;
