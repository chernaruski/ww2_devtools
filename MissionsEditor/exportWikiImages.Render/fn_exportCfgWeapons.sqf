...
_mode = _this param [0,"Weapon",[""]];
_patches = _this param [1,[],[[]]];
_types = _this param [2,[],[[]]];
	...
	if (
		...
		if (_screenshots) then {
			...
			if (_capture) then {
				closeDialog 0;
				sleep 1;
				//screenshot format ["CfgWeapons_%1.png", _class]; //vanilla screenshot command images seem too washed out
