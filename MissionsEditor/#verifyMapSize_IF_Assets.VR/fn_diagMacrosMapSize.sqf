...
_configClasses = _this select 0;
_mode = _this param [1,"config",[""]];
_exclude = _this param [2,[],[[]]];
...
_cfgCount = count _configClasses - 1;
_pos = [100,100,100];
...
//	progressLoadingScreen ((_c / (_cfgCount max 1)) / 2);
	hint format ["Phase one: %1 / %2", _c, (_cfgCount max 1)];
...
//	progressLoadingScreen (0.5 + ((_c / (_classCount max 1)) / 2));
	hint format ["Phase two: %1 / %2", _c, (_classCount max 1)];
...
//endLoadingScreen;
hint "Finished diagMacrosMapSize";
_resultText