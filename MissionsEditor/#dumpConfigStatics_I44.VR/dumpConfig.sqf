...
"ConfigDumpFileIO" callExtension ("open:" + "A3_Statics_I44.cpp");
...
//		push(_result,(_indents select _depth) + _this);
	...
	_traverse =
	{
		...
		{
			private ["_skipClass","_skipContent"];
			_skipClass = false;
			_skipContent = false;
...
//					_skipClass = false;
				...
				{
//					if ((_confName isKindOf "AllVehicles") || (_depth != 1)) 
					if ((_confName isKindOf "Static") || (_confName isKindOf "Thing") || (_confName isKindOf "FloatingStructure_F") || (_depth != 1)) then
					{
						private["_author"];
						_author = toLower (getText (configFile/"CfgVehicles"/_confName/"author"));
			...
			if (!(_skipClass)) then
			{
				...
				(
					...
					{
						if ((_this == "") || (!(_specifyParentClass))) then
						{
							""
						}
						else
...
private ["_return","_startTime","_endTime","_text"];

startLoadingScreen [""];
_startTime = diag_tickTime;
_return = _this call _dumpConfigTree;
_endTime = diag_tickTime;
//copyToClipboard _return;
endLoadingScreen;

_text = format["Done in %1 seconds",_endTime - _startTime];
hint _text;
diag_log [_text];
