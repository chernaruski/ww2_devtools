/*
	...
	Returns:
   	...
   	8: _randomizeAnims:bool - true if vehicle supports randomization through animations (default: false)
   	9: _randomizeTexs:bool - true if vehicle supports randomization of textures (default: false)
...
#define RETURN_VALUES					[_class,_model,_reversed,_verticalOffset,_animationsAdjustments,_selectionsToHide,_verticalOffsetWorld,_textures,_randomizeAnims,_randomizeTexs]
...
private _textures = [];
private _useInit = [];
private _randomizeAnims = false;
private _randomizeTexs = false;
...
if (_input isEqualType objNull) then
{
	...
	if (_input isKindOf "ship") then
	{
		_verticalOffset = ROUND_DECIMALS(((getPosWorld _input) select 2) - ((getPosASL _input) select 2), 0.001);
	}
	else
	{
	...
	//check if object supports multiple texture randomization
	_randomizeTexs = count getArray(configFile >> "CfgVehicles" >> typeOf cursorObject >> "textureList") > 2;
