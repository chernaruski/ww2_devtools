...
Test_fnc_getClassProperties =
{
	...
	private _reversed = getNumber(configFile >> "CfgVehicles" >> _class >> "reversed");
	private _selections = getArray(configFile >> "CfgVehicles" >> _class >> "hiddenSelections");
//	private _textures = getArray(configFile >> "CfgVehicles" >> _class >> "hiddenSelectionsTextures");

	private _animationSources = configFile >> "CfgVehicles" >> _class >> "AnimationSources";
	private _animationSourcesList = [];

	{
		private _source = getText(configfile >> "CfgVehicles" >> _class >> "AnimationSources" >> configName _x >> "source");
		_animationSourcesList pushBack (toLower _source);
	} forEach (_animationSources call bis_fnc_returnChildren);

	private _textureList = getArray(configFile >> "CfgVehicles" >> _class >> "textureList");

	_selections = _selections apply {toLower _x};
//	_textureList = _textureList apply {toLower _x};

	_classProperties pushBack (toLower _model);
	_classProperties pushBack (_reversed);
	_classProperties pushBack (_selections);
	_classProperties pushBack (_animationSourcesList);
	_classProperties pushBack (_textureList);
...
//get list of class names that will be processed
{
	...
	private _author = toLower (getText (configFile >> "CfgVehicles" >> _class >> "author"));

	if ((_author in TEST_IncludedAuthors) || true) then
	{
		private _classPropertiesCurrentClass = [_class] call Test_fnc_getClassProperties;
		private _parentClass = configName (inheritsFrom (configFile >> "CfgVehicles" >> _class));
		private _classPropertiesParentClass = [_parentClass] call Test_fnc_getClassProperties;

//diag_log ["_classPropertiesCurrentClass",_class,_classPropertiesCurrentClass];
//diag_log ["_classPropertiesParentClass",_parentClass,_classPropertiesParentClass];
//diag_log ["isEqualTo",_classPropertiesCurrentClass isEqualTo _classPropertiesParentClass];

		if (!(_classPropertiesCurrentClass isEqualTo _classPropertiesParentClass)) then
		{
			_result pushBack format ["%1",_class];
		};
	};
...
//		};
		...
		if ({_class isKindOf _x} count ["air","ship","tank","car","staticweapon"] > 0) then
		{
			...
			_object setVelocity [0,0,-3];
			sleep 3;//need to wait for simulation to kick-in
		};
...
//useful?
//		if ({_verticalOffsetAsl == 0 && {_verticalOffsetWorld == 0 && {_animateCount == 0 &&  {_hideCount == 0}}}}) exitWith {};
...
//			"#define " + _pathText + "_SIMPLEOBJECT_" + _x + _brMacro +
			"	class " + _x + ": " + configName (inheritsFrom (configFile >> "CfgVehicles" >> _x)) + _brMacro +
			"	{" + _brMacro +
		...
		"ConfigDumpFileIO" callExtension ("write:" + _singleEntry);
