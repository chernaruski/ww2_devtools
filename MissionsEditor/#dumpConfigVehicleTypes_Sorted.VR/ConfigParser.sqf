...
if (count _this == 0) exitWith {_dbg = _dbg + " Empty array supplied."; diag_log text _dbg; _dbg};
{
	...
	{
		case 0: {if (isNil("_x") || {!(typeName(_x) == "STRING")}) then {
					_dbg = _dbg + " Parameter 0 isn't a class."};};
...
_myInitString = format ["A3_%1_sorted.cpp",_vehicleType];
...
*/
			AddToOutput(_tabs + _TAB + configName(_x) + "[] = " + (getArray _x call _traverseArray) + ";")};
		if isNumber(_x) then {
...
for "_i" from (0) to ((count(configFile/_rootClass)) - 1) do
{
	...
	if (isClass _class) then
	{
		...
		if (_className isKindOf _vehicleType) then
		{
