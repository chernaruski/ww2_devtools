...
//			(_className isKindOf ["Launcher_Base_F",configFile/_rootClass]) or
			...
			(_className isKindOf ["LIB_Binocular_base",configFile/_rootClass]) or
//			(_className isKindOf ["LIB_ACC_base",configFile/_rootClass]) or
...
for "_i" from 0 to (_numberOfWeapons - 1) do
{
	...
	_tag = switch (true) do
	{
		case (_weaponClass isKindOf ["LIB_Binocular_base",configFile/_rootClass]): {"Item_"};
//		case (_weaponClass isKindOf ["LIB_ACC_base",configFile/_rootClass]): {"Item_"};
		default {"Weapon_"};
	};
